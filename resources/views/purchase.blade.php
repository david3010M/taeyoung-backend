    ...
    <tr class="w100">
        ...
        <td class="w100">
            <table class="w100 pe64">
                <tr>
                    <td>
                        <h2 class="m0 text-end normal font20">COMPRA <strong class="">N° {{$purchase->number}}</strong>
                        </h2>
                    </td>
                </tr>
                <tr>
                    <td class="text-end interletter bold">
                        {{ Carbon::parse($purchase->date)->locale('es')->isoFormat('D [de] MMMM [de] YYYY') }}
                    </td>
                </tr>
            </table>
        </td>
    ...
    <tr class="w100">
        <td class="w100">
            <table class="w100 pl56">
                <tr>
                    <td>
                        <h2 class="m0 text-start normal font20"><strong>{{ $purchase->supplier->filterName }}</strong>
                        </h2>
                    </td>
                </tr>
                <tr>
                    <td class="text-start bold font14 gray">
                        {{ $purchase->supplier->filterDocument }}
                    </td>
                </tr>
            </table>
    </tr>
...
<div class="mx56">
    ...
    <table class="w100 tableData mb40">
        ...
        @foreach ($purchase->detailMachinery as $index => $machinery)
            <tr class="text-center font10">
                <td>{{ $machinery->description }}</td>
                <td>{{ $machinery->quantity }}</td>
                <td>{{$purchase->currencySymbol}} {{ $machinery->purchasePrice }}</td>
                <td>{{$purchase->currencySymbol}} {{ $machinery->purchaseValue }}</td>
            </tr>
        @endforeach
        @foreach ($purchase->detailSpareParts as $index => $sparePart)
            <tr class="text-center font10" style=background-color:#f4f4f4>
                <td>{{ $sparePart->sparePart->name }}</td>
                <td>{{ $sparePart->quantity }}</td>
                <td>{{$purchase->currencySymbol}} {{ $sparePart->purchasePrice }}</td>
                <td>{{$purchase->currencySymbol}} {{ $sparePart->purchaseValue }}</td>
            </tr>
        @endforeach
    </table>
                    ...
                    <tr>
                        <td>
                            <h2 class="m0 text-end normal font10">Total maquinaria:
                                <strong>
                                    {{$purchase->currencySymbol}} {{$purchase->totalMachinery}}
                                </strong>
                            </h2>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <h2 class="m0 text-end normal font10">Total repuestos:
                                <strong>
                                    {{$purchase->currencySymbol}} {{$purchase->totalSpareParts}}
                                </strong>
                            </h2>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <h2 class="m0 text-end normal font10">Subtotal:
                                <strong>
                                    {{$purchase->currencySymbol}} {{$purchase->subtotal}}
                                </strong>
                            </h2>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td>
                            <h2 class="m0 text-end normal font10">Total:
                                <strong>
                                    {{$purchase->currencySymbol}} {{$purchase->total}}
                                </strong>
                            </h2>
                        </td>
                    </tr>
